...
function query(filterBy = { name: '' }) {
    ...
    if (filterBy.label) {
        toysToReturn = toysToReturn.filter(toy => toy.labels.includes(label))
    }
    if (filterBy.sortByName) {
        toysToReturn = toysToReturn.sort(toy1,toy2 => (toy1.name.localeCompare(toy2.name)))
    }
    if (filterBy.sortByPrice) {
        toysToReturn = toysToReturn.sort(toy1,toy2 => (toy1.price-toy2.price))
    }
    if (filterBy.sortByCreated) {
        toysToReturn = toysToReturn.sort(toy1,toy2 => (toy1.createdAt-toy2.createdAt))
    }
